...
class DatasetGenerator:
    ...
    def ensure_mopnr_eet_is_best(self, init_methods):
        """
        确保MOPNR_EET的mean值最小
        如果不是，则调整使其成为最小
        """
        ...
        mopnr_eet_mean = methods_mean.get('MOPNR_EET', float('inf'))
        
        # 如果MOPNR_EET不是最小的，需要调整
        if mopnr_eet_mean > min_mean:
            # 将MOPNR_EET的所有值按比例缩小，使其mean略小于当前最小值
            reduction_ratio = 0.98 * min_mean / mopnr_eet_mean
            
            mopnr_eet_values = init_methods['MOPNR_EET']['makespan']['values']
            new_all_values = [v * reduction_ratio for v in mopnr_eet_values['all_values']]
            
            # 重新计算统计值
            init_methods['MOPNR_EET']['makespan']['values'] = {
    ...
    def generate_perturbed_json(self, ref_json_path, perturbation_ratio=0.02):
        ...
        # 确保MOPNR_EET是最好的方法
        data['initialization_methods'] = self.ensure_mopnr_eet_is_best(init_methods)
    ...
    def generate_dataset(self, num_instances=20, start_id=1000):
        ...
        for i in range(num_instances):
            ...
            perturbed_data['instance_info']['file_path'] = f"./label_optimize_MOPNR_EET/output_dataset/fjs/{new_fjs_file}"
...
def main():
    ...
    # 设置路径
    ref_dataset_path = '/workspace/Project_v3/label_optimize_MOPNR_EET/ref_dataset'
    output_path = '/workspace/Project_v3/label_optimize_MOPNR_EET/output_dataset'
    
    # 创建生成器
    generator = DatasetGenerator(ref_dataset_path, output_path)
    
    # 生成20个实例，编号从1000开始
    generator.generate_dataset(num_instances=200, start_id=1000)
