...
def compute_pareto_front_enhanced_label(label_info, rule_names, label_name="mean",
                                        front_weight=0.75, decay_rate=2.0):
    """
    方案1：基于Pareto前沿分层的概率分配标签生成（推荐）
    
    核心思想：
    1. 显式识别Pareto前沿（rank=0）的规则
    2. 前沿内按拥挤距离分配大部分概率（鼓励多样性）
    3. 被支配规则按rank指数衰减分配少量概率（保持探索）
    4. 让GNN学习"预测前沿集合"而非"选单一最优"
    
    相比当前方法的改进：
    - 显式区分Pareto前沿和被支配解
    - 保留分布信息（前沿内多规则均有较高概率）
    - 匹配NSGA2的选择逻辑（rank优先 + crowding distance多样性）
    ...
    label_name : str, default="mean"
        使用的统计量名称 ("mean", "median", "min", etc.)
    front_weight : float, default=0.75
        Pareto前沿的概率权重（推荐0.7-0.8）
        - 0.75表示75%的概率分配给前沿规则，25%分配给被支配规则
        - 越高表示越信任前沿规则，但会降低探索性
    decay_rate : float, default=2.0
        被支配解的rank衰减率（越大衰减越快）
        - rank=1的规则概率为 exp(-2.0*1) ≈ 0.135
        - rank=2的规则概率为 exp(-2.0*2) ≈ 0.018
    
    返回:
    --------
    torch.Tensor, shape (len(rule_names),)
        负对数概率向量（越小表示越推荐）
        训练时会通过 F.softmax(-y / temperature) 恢复概率分布
    
    示例:
    --------
    假设有7个规则，Pareto前沿有3个（rank=0），被支配解4个（rank=1,1,2,3）
    
    步骤1：前沿内按拥挤距离分配
        规则A (cd=0.8): 0.8/2.0 * 0.75 = 0.30
        规则B (cd=0.7): 0.7/2.0 * 0.75 = 0.26
        规则C (cd=0.5): 0.5/2.0 * 0.75 = 0.19
    
    步骤2：被支配解按rank衰减
        规则D (rank=1): exp(-2*1)/X * 0.25 ≈ 0.08
        规则E (rank=1): exp(-2*1)/X * 0.25 ≈ 0.08
        规则F (rank=2): exp(-2*2)/X * 0.25 ≈ 0.05
        规则G (rank=3): exp(-2*3)/X * 0.25 ≈ 0.04
    
    步骤3：返回负对数概率
        y = [-log(0.30), -log(0.26), -log(0.19), -log(0.08), ...]
          = [1.20, 1.35, 1.66, 2.53, ...]
    
    训练时恢复：
        probs_recovered = softmax(-y / 0.02) ≈ 原始概率分布
    ...
    # 6. 分层概率分配
    probs = np.zeros(n_rules, dtype=float)
    
    # 6.1 识别Pareto前沿（rank=0）
    pareto_front_indices = np.where(pareto_ranks == 0)[0]
    
    if len(pareto_front_indices) > 0:
        # 前沿内按拥挤距离分配概率（拥挤距离大=多样性好=概率高）
        front_cd = crowding_distances_norm[pareto_front_indices]
        front_cd_sum = np.sum(front_cd)
        
        if front_cd_sum > 0:
            # 归一化并分配 front_weight 比例的总概率
            front_probs = (front_cd / front_cd_sum) * front_weight
            probs[pareto_front_indices] = front_probs
        else:
            # 如果所有拥挤距离相同，均匀分配
            uniform_prob = front_weight / len(pareto_front_indices)
            probs[pareto_front_indices] = uniform_prob
    
    # 6.2 被支配解（rank>0）按rank指数衰减
    dominated_indices = np.where(pareto_ranks > 0)[0]
    
    if len(dominated_indices) > 0:
        # 指数衰减：exp(-decay_rate * rank)
        dominated_ranks = pareto_ranks[dominated_indices]
        dominated_scores = np.exp(-decay_rate * dominated_ranks)
        dominated_scores_sum = np.sum(dominated_scores)
        
        if dominated_scores_sum > 0:
            # 归一化并分配 (1 - front_weight) 比例的总概率
            dominated_probs = (dominated_scores / dominated_scores_sum) * (1 - front_weight)
            probs[dominated_indices] = dominated_probs
        else:
            # 极端情况：均匀分配
            uniform_prob = (1 - front_weight) / len(dominated_indices)
            probs[dominated_indices] = uniform_prob
    ...
    if probs_sum > 0:
        ...
        # 极端情况：所有规则均匀分配
        probs = np.ones(n_rules) / n_rules
    
    # 8. 转换为负对数概率（越小越好）
    # 添加 epsilon 避免 log(0)
    epsilon = 1e-10
    neg_log_probs = -np.log(probs + epsilon)
    
    # 9. 返回 torch.Tensor（与当前方法接口一致）
    # 训练代码会对此进行 F.softmax(-y / temperature) 恢复概率分布
    return torch.tensor(neg_log_probs, dtype=torch.float32)
...
# ==================== 0129 新 多目标加权生成单标签的方案 ====================

def compute_weighted_objective_label(label_info, rule_names, label_name="mean",
                                     makespan_weight=0.4, max_load_weight=0.3, 
                                     total_load_weight=0.3, normalize=True):
    """
    基于三个目标的加权平均计算单标签（多目标简化版）
    
    核心思想：
    1. 提取每个规则的三个目标值（makespan, max_machine_load, total_machine_load）
    2. 对三个目标进行归一化（避免量级差异导致某个目标主导结果）
    3. 计算加权平均得分：weighted_score = w1*makespan + w2*max_load + w3*total_load
    4. 返回log(score + 1)，与原单目标方法接口完全一致
    ...
    label_name : str, default="mean"
        使用的统计量名称 ("mean", "median", "min", etc.)
    makespan_weight : float, default=0.4
        makespan的权重（推荐0.3-0.5）
    max_load_weight : float, default=0.3
        max_machine_load的权重（推荐0.2-0.4）
    total_load_weight : float, default=0.3
        total_machine_load的权重（推荐0.2-0.4）
    normalize : bool, default=True
        是否在加权前对目标值进行归一化（强烈推荐开启，避免量级差异）
    
    返回:
    --------
    torch.Tensor, shape (len(rule_names),)
        加权得分向量（越小越好），格式为 log(score + 1)
        训练代码会对此进行 F.softmax(-y / temperature)
    
    示例:
    --------
    假设有3个规则的原始目标值：
        规则A: makespan=100, max_load=20, total_load=60
        规则B: makespan=90,  max_load=25, total_load=70
        规则C: makespan=110, max_load=18, total_load=55
    ...
    步骤2：加权平均（权重0.4, 0.3, 0.3）
        规则A: 0.4*0.5 + 0.3*0.29 + 0.3*0.33 = 0.386
        规则B: 0.4*0.0 + 0.3*1.0 + 0.3*1.0 = 0.600
        规则C: 0.4*1.0 + 0.3*0.0 + 0.3*0.0 = 0.400
    
    步骤3：log变换
        y = [log(1.386), log(1.600), log(1.400)] = [0.326, 0.470, 0.336]
    ...
    Notes:
    ------
    - 归一化使用min-max方法，将每个目标缩放到[0,1]
    - 归一化后，三个目标在同一量级，权重才有实际意义
    - 如果不归一化，makespan（数值通常较大）会主导结果
    - 权重之和建议为1.0，便于解释
    """
    # 1. 提取每个规则的三个目标值
    objectives = []
    ...
    objectives = np.array(objectives, dtype=float)  # shape: (n_rules, 3)
    
    # 2. 归一化（min-max归一化到[0,1]）
    if normalize:
        obj_min = np.min(objectives, axis=0)  # shape: (3,)
        obj_max = np.max(objectives, axis=0)  # shape: (3,)
        obj_range = obj_max - obj_min
        
        # 避免除零：如果某个目标所有规则值相同，范围为0
        for i in range(3):
            if obj_range[i] == 0:
                obj_range[i] = 1.0
    ...
    # 3. 计算加权平均
    weights = np.array([makespan_weight, max_load_weight, total_load_weight])
    weighted_scores = np.dot(objectives_norm, weights)  # shape: (n_rules,)
    
    # 4. 返回log(score + 1)，与原方法接口一致
    # 训练代码会对此进行 F.softmax(-y / temperature)
    log_scores = torch.log(torch.tensor(weighted_scores, dtype=torch.float32) + 1.0)
    
    return log_scores

# ==================== 0129 新 多目标加权生成单标签的方案  结束 ====================
    ...
    def __init__(self, fjs_root_path, label_root_path, *, label_name = "mean", device = None, train_ratio = 0.8):
        ...
        for label_path in labels:
            ...
            try:
                ...
                # 新的单标签
                # # ============== 多目标标签计算（方案1：Pareto前沿强化） ==============
                # 核心改进：
                # 1. 显式识别Pareto前沿（rank=0）并分配大部分概率（75%）
                # 2. 前沿内按拥挤距离分配，鼓励多样性
                # 3. 被支配规则保留少量概率（25%），保持探索性
                # 4. 让GNN学会"预测前沿集合"而非"选单一最优"
                # 
                # 优势：
                # - 相比BestSingleRule：初始化包含多个高质量前沿规则（质量+多样性）
                # - 相比Random：高质量起点，快速收敛到真Pareto前沿
                # - 相比当前方法：显式利用Pareto前沿信息，避免信息压缩损失
                # g.y = compute_pareto_front_enhanced_label(
                #     label_info=label_info,
                #     rule_names=rule_names,
                #     label_name=self.label_name,
                #     front_weight=0.75,       # Pareto前沿概率权重（推荐0.7-0.8）
                #     decay_rate=2.0           # 被支配解衰减率（推荐1.5-3.0）
                # )

                # 新加权单标签方案
                g.y = compute_weighted_objective_label(
                    label_info=label_info,
                    rule_names=rule_names,
                    label_name=self.label_name,
                    makespan_weight=0.4,      # makespan权重（推荐0.3-0.5）
                    max_load_weight=0.5,      # max_machine_load权重（推荐0.2-0.4）
                    total_load_weight=0.1,    # total_machine_load权重（推荐0.2-0.4）
                    normalize=False            # 是否归一化（强烈推荐开启）
                )
            except KeyError as e:
