...
def get_best_method(label_info, label_name="mean", objective=None):
    """
    获取最佳初始化方法（与dataset.py中的规则一致）
    返回: 0-7的索引，对应8种初始化方法
    """
    makespans = np.array([
        label_info["FIFO_SPT"][objective]["values"][label_name],
        label_info["FIFO_EET"][objective]["values"][label_name],
        label_info["MOPNR_SPT"][objective]["values"][label_name],
        label_info["MOPNR_EET"][objective]["values"][label_name],
        label_info["LWKR_SPT"][objective]["values"][label_name],
        label_info["LWKR_EET"][objective]["values"][label_name],
        label_info["MWKR_SPT"][objective]["values"][label_name],
        label_info["MWKR_EET"][objective]["values"][label_name]
    ])
    # 取log后再argmin（与dataset.py第43-74行一致）
    log_makespans = np.log(makespans + 1)
    return np.argmin(log_makespans)

def analyze_label_distribution(label_root_path, label_name="mean", objective=None):
    """
    分析标签分布
    返回: 每个标签对应的实例列表和统计信息
    """
    ...
    for label_path in label_files:
        try:
            ...
            best_method_idx = get_best_method(label_info, label_name, objective)
            best_method_name = METHOD_NAMES[best_method_idx]
            
            # 记录详细信息
            makespans = [
                label_info[method][objective]["values"][label_name]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_txt', type=str, default='label_distribution_report_4.txt',
                        help='文本报告输出文件名')
