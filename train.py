...
class Trainer:
    ...
    def plot_training_history(self):
        ...
        axes[0, 0].fill_between(epoch_idx, train_loss_lower, train_loss_upper, 
                                color='blue', alpha=0.25)
        
        # 绘制验证损失曲线和阴影
        axes[0, 0].plot(epoch_idx, val_loss_array, 'r-', label='Val Loss', linewidth=2)
        axes[0, 0].fill_between(epoch_idx, val_loss_lower, val_loss_upper, 
                                color='red', alpha=0.25)
        
        axes[0, 0].set_xlabel('Epoch', fontsize=12)
        axes[0, 0].set_ylabel('Loss', fontsize=12)
        axes[0, 0].set_title('Training Loss vs Validation Loss (±1 Std)', fontsize=14, fontweight='bold')
