...
class DatasetGenerator:
    ...
    def ensure_fifo_spt_is_best(self, init_methods):
        """
        确保FIFO_SPT的mean值最小
        如果不是，则调整使其成为最小
        """
        ...
        fifo_spt_mean = methods_mean.get('FIFO_SPT', float('inf'))
        
        # 如果FIFO_SPT不是最小的，需要调整
        if fifo_spt_mean > min_mean:
            # 将FIFO_SPT的所有值按比例缩小，使其mean略小于当前最小值
            reduction_ratio = 0.98 * min_mean / fifo_spt_mean
            
            fifo_spt_values = init_methods['FIFO_SPT']['makespan']['values']
            new_all_values = [v * reduction_ratio for v in fifo_spt_values['all_values']]
            
            # 重新计算统计值
            init_methods['FIFO_SPT']['makespan']['values'] = {
    ...
    def generate_perturbed_json(self, ref_json_path, perturbation_ratio=0.02):
        ...
        # 确保FIFO_SPT是最好的方法
        data['initialization_methods'] = self.ensure_fifo_spt_is_best(init_methods)
